...
The first thing to notice is the reduced size for jar that was _minified_
by R8.

Now the issue is to pass this minified jar to `native-image`, gradle plugin
usually pass a main class and a classpath. The custom `r8` task is a `JavaExec`
and cannot be used directly with `nativeImage.jarTask`.
